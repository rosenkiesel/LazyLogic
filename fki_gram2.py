class HistGen:
  ...
  def __iter__(self):
    return Abo(self)
  def fetch_next(self):
    self.generated.append( self.gen.next() )
...
class Abo:
  ...
  def __iter__(self):
    return self
  def next(self):
    while len(self.hist.generated) <= self.idx:
      self.hist.fetch_next()
    val=self.hist.generated[self.idx]
    self.idx = 1+self.idx
    return val
...
class lazyLogic:
   ...
   def logicCall(self):
     self.simplify = self.simplify.redirectMe(self)

   def redirectMe(self, handle):
     if not(self.simplify is self):
       self.listeners -= handle
       if self.simplify is lateLogic:
         self.simplify.listeners += handle
   ...
   def __not__(self):
     return lazyLogic_Not(self,other)

def lazyLogicVisitor(logic, recurVal, recurDict):
   if logic in recurDict:
     return recurDict[logic]
   recurDict[logic]=recurVal
   if logic is lazyLogic_And:
     a = lazyLogicVisitor(logic.a, recurVal, recurDict)
     b = lazyLogicVisitor(logic.b, recurVal, recurDict)
     ret = 
   recurDict.remove(logic)

#class Grammar:
#
#  def findAllMatches(self,seq):
#    seq has to be fully read  
#    all possible matchings are given
#
#  def uniqueStreamMatch(self,stream):
#    may raise NotUnique
#    may give partially parsed subGrammars 
